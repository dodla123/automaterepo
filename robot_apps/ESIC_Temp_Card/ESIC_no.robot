...
${DOWNLOAD_FOLDER}            ${TEMP_DIRECTORY_PATH}/Downloaded_Temp_cards/    
...
${TAGGED_EMAIL}               dodla.manasa@buzzworks.com    #lakshmi.l@buzzworks.com    #shashwat.b@buzzworks.com      #bandari.akhil@buzzworks.com     
...
Click Continue Button for ESIC No
    ...
    IF    '${Element_exist}' == 'True'
        Condition if Already Tagged in ESIC No    ${row}
    ELSE
...
Condition for Upadation of Bank Details
    ...
    Set Cell Value    ${row}    AK    Updated Sucessfully
    Set Cell Value    ${row}    AI    ${current_date}
    Set Cell Value    ${row}    AJ    Success
    Save Workbook
    Close Workbook
    Close Window
    [Return]    ${row}
...
Updating the Bank Details for YES 
    ...
    ${element_disabled_P}    Execute Javascript    return document.getElementById('ctl00_HomePageContent_rdPersonalDetails').disabled
    Log    ${element_disabled_P}
    ${element_disabled_B}    Execute Javascript    return document.getElementById('ctl00_HomePageContent_rdBankDetails').disabled
    Log    ${element_disabled_B}
...
Condition if Already Tagged in ESIC No
    ...
    Log    Current Date: ${current_date}
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AK    Already Tagged
    Set Cell Value    ${row}    AI    ${current_date}
    Set Cell Value    ${row}    AJ    Success
    Save Workbook
    Close Workbook
...
Condition for Nominee details if None
    ...
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AK    Nominee Name not provided
    Set Cell Value    ${row}    AJ    Pending
    Save Workbook
    Close Workbook
    Close Window
    [Return]    ${row}
...
Condition for Nominee relation details if None
    ...
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AK    Nominee relation not provided
    Set Cell Value    ${row}    AJ    Pending
    Save Workbook
    Close Workbook
    Close Window
...
Condition for Bank details is None
    ...
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AK    Bank details not provided
    Set Cell Value    ${row}    AJ    Pending
    Save Workbook
    Close Workbook
    Close Window
...
Condition for Account No if POP-UP 
    ...
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AK    Bank Account is Already Tagged    
    Set Cell Value    ${row}    AJ    Pending  
    Save Workbook
    Close Workbook
    Close Window
...
Condition for Bank if IFSC not found
    ...
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AK    Bank IFSC not Found
    Set Cell Value    ${row}    AJ    Pending
    Save Workbook
    Close Workbook  
    Close Window
...
Condition for Next Steps 
    ...
    Move File    ${DOWNLOAD_PATH}/${ESIC_NO_Created}.pdf    ${DOWNLOAD_FOLDER}/${NumGen}[Name] - ${ESIC_NO_Created}.pdf
    ...
    Log    Current Date: ${current_date}
    Open Workbook    ${EXCEL_FILE_PATH}
    Set Cell Value    ${row}    AH    ${ESIC_NO_Created}
    Set Cell Value    ${row}    AI    ${current_date}
    Set Cell Value    ${row}    AJ    Success
    Save Workbook
    Close Workbook
...
Send Email With Pending Data
    ...
    Create Workbook    ${TEMPDIR}${/}Pending_Data.xlsx
    Open Workbook    ${excel_file_path}
    Append Rows To Worksheet    ${excel_data}    Sheet1
    Save Workbook    ${TEMPDIR}${/}Pending_Data.xlsx
    ...
    Send Message     ${EMAIL_SENDER}    ${TAGGED_EMAIL}    ${SUBJECT}    ${email_body}    attachments=${TEMPDIR}${/}Pending_Data.xlsx

    # Remove the temporary Excel file
    Remove File    ${TEMPDIR}${/}Pending_Data.xlsx
            ...
            FOR    ${temp}    IN    @{temp_list}
                ...
                IF    '${user}' != 'None'
                    ...
                    IF    '${NumGen}[ESIC No]' != 'None'
                        ...
                        Click Continue Button for ESIC No    ${row} 
